...
void preencher_tracinhos(vector<vector<string>>& tracinhos, arvore* l, int nivel, int pos, int espaco) {
    if (l == nullptr) {
        return;
    }

    if (l->esquerda) {
        tracinhos[nivel][pos - espaco / 2] = '/';
        preencher_tracinhos(tracinhos, l->esquerda, nivel + 1, pos - espaco, espaco / 2);
    }
    if (l->direita) {
        tracinhos[nivel][pos + espaco / 2] = '\\';
        preencher_tracinhos(tracinhos, l->direita, nivel + 1, pos + espaco, espaco / 2);
    }
}

void imprimir_arvore(arvore* l) {
    int altura = calcular_altura(l);
    int largura = pow(2, altura) * 4; // Ajuste para melhor espaçamento

    vector<vector<string>> niveis(altura, vector<string>(largura, " "));
    vector<vector<string>> tracinhos(altura - 1, vector<string>(largura, " "));

    preencher_niveis(l, niveis, 0, largura / 2, largura / 4);
    preencher_tracinhos(tracinhos, l, 0, largura / 2, largura / 4);
